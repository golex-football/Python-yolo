...
from services.connection.bt_model_pb import YoloPacket, PF_BGR24, PF_GRAY8
...
def main():
    ...
    OUT_MODE = _env("MOONALT_OUT_MODE", "bind").lower().strip()  # bind|connect (default bind as you requested)
    ...
    while True:
        try:
            raw = pull.recv()
            print(f"[worker] recv bytes={len(raw)}", flush=True)  # Nazari: log right after receive

            msg = InputFrame()
            try:
                msg.ParseFromString(raw)
            except Exception as e:
                print(f"[worker][ERR] ParseFromString failed: {e!r}", flush=True)
                continue

            print(f"[worker] parsed: frame_id={msg.frame_id} schema={msg.schema} w={msg.width} h={msg.height} pix={msg.pixel_format} data={len(msg.frame_data)}", flush=True)  # Nazari: log after parse
            ...
            h = int(msg.height)
            expected = w * h * 3
            if len(msg.frame_data) != expected:
                print(f"[worker][ERR] frame_data size mismatch: got={len(msg.frame_data)} expected={expected}", flush=True)
                continue

            frame = np.frombuffer(msg.frame_data, dtype=np.uint8).reshape((h, w, 3))
            ...
            # raw_frame (BroadTrack schema)
            out.raw_frame.width = w
            out.raw_frame.height = h
            out.raw_frame.pixel_format = PF_BGR24
            out.raw_frame.frame_data = msg.frame_data  # BGR24 bytes

            # mask_frame (BroadTrack schema)
            out.mask_frame.width = w
            out.mask_frame.height = h
            out.mask_frame.pixel_format = PF_GRAY8
            out.mask_frame.frame_data = mask.tobytes()  # GRAY8 bytes

            # box_frame (BroadTrack schema)
            out.box_frame.width = w
            out.box_frame.height = h

            # convert from xywh-center to top-left uint box
            if getattr(r0, "boxes", None) is not None and r0.boxes is not None and len(r0.boxes) > 0:
                xywh = r0.boxes.xywh.detach().cpu().numpy()
                conf = r0.boxes.conf.detach().cpu().numpy()
                cls  = r0.boxes.cls.detach().cpu().numpy()
                for (cx, cy, bw, bh), sc, ci in zip(xywh, conf, cls):
                    x = int(max(0.0, cx - bw / 2.0))
                    y = int(max(0.0, cy - bh / 2.0))
                    ww = int(max(0.0, bw))
                    hh = int(max(0.0, bh))
