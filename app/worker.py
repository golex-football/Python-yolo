...
def main():
    ...
    while True:
        try:
            ...
            # Try BroadTrack CapturePacket first (optional), otherwise fall back to InputFrame
            frame_id = None
            ts_ns = time.time_ns()
            raw_bgr_bytes = None
            w = h = None
            ...
            if cap is not None and cap.raw_frame is not None and len(cap.raw_frame.frame_data) > 0 and cap.raw_frame.width > 0 and cap.raw_frame.height > 0:
                frame_id = int(cap.frame_id)
                ts_ns = int(cap.raw_frame.timestamp) if getattr(cap.raw_frame, "timestamp", 0) else ts_ns
