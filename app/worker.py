...
from services.connection.yolo_packet_pb2 import YoloPacket  # CHANGED
...
IN_EP  = _norm_ipc("MOONALT_IPC_IN", "ipc:///tmp/capture")
OUT_EP = _norm_ipc("MOONALT_BROADTRACK_OUT", "ipc:///tmp/broadtrack_in.sock")

# enums (numeric) from frame_message.proto
...
def run_worker():
    ...
    # Capture side binds PUSH -> YOLO connects PULL
    pull = ZMQConnectPull(ctx, IN_EP)
    # BroadTrack side binds PULL -> YOLO connects PUSH
    push = ZMQConnectPush(ctx, OUT_EP)
    print(f"[worker] READY. IN(connect PULL)={IN_EP}  OUT(connect PUSH)={OUT_EP}")
    ...
    try:
        while True:
            ...
            # Accept both old-string and new-enum formats
            pf_ok = (getattr(msg, "pixel_format", "") == "BGR24") or (getattr(msg, "pixel_format_id", 0) == PIXEL_FORMAT_BGR24)
            if not pf_ok:
                continue
            ...
            # --------- YoloPacket instead of BtPacket ---------
            pkt = YoloPacket()
            ...
            if SEND_RAW:
                ...
                pkt.raw_frame = msg.frame_data   # unchanged pass-through

            # --- mask frame ---
            mh, mw = mask_gray.shape[:2]
            pkt.mask_width = mw
            pkt.mask_height = mh
            pkt.mask_frame = mask_gray.tobytes()  # GRAY8, 1-channel
            ...
            for (x1, y1, x2, y2), s, c in zip(boxes_xyxy, scores, classes):
                ...
                pkt.boxes.append(x)
                pkt.boxes.append(y)
                pkt.boxes.append(w_box)
                pkt.boxes.append(h_box)
            ...
            if (frame_counter % max(1, LOG_EVERY)) == 0:
                ...
                print(f"[worker] frame_id={msg.frame_id} size=({w},{h}) "
                      f"mask=({h},{w}) boxes={len(boxes_xyxy)} proc_fps={pf:.1f}")
